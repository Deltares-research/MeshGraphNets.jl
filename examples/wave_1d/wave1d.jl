    ...
    if ind in train_inds
        ...
        jldopen(joinpath(datadir, "train.jld2"), "a") do f
            ...
            for idx in axes(mesh_pos,2)
                traj_group["node[$idx].mesh_pos"] = mesh_pos[:,idx]
                traj_group["node[$idx].node_type"] = node_type[idx]
                traj_group["node[$idx].d"] = D_center[idx]
...
meta_dict = Dict(
    "dt" => Float32(dt_out),
    "trajectory_length" => length(times),
    "dims" => 2,
    "n_nodes" => "n_nodes",
    "edges" => Dict(
        "key" => "edges",
    ...
    "feature_names" => ["mesh_pos", "node_type", "bathymetry", "velocity", "waterlevel"],
    "target_features" => ["velocity", "waterlevel"],
    "features" => Dict(
        "mesh_pos" => Dict(
            "key" => "node[%d].mesh_pos",
            "type" => "static",
...
open(joinpath(datadir, "meta.json"), "w") do file
    JSON3.pretty(file, meta_dict)
end
